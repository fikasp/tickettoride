...
const Modal = ({ info, visibility }) => {
	...
	return (
		<div className="modal" style={{ zIndex: 1 }}>
			<div className="modal_content" onKeyDown={handleKeyDown} tabIndex={0} ref={ref}>
				{info}
...
const Scores = ({ players, visibility }) => {
	...
	return (
		<div className="modal" style={{ zIndex: 1 }}>
			<div ref={ref} tabIndex={0} className="modal_content modal_content-score" onKeyDown={handleKeyDown}>
...
const EditTrains = ({ trains, setTrains, setTrainsScore, visibility, color }) => {
	...
	const trainsScore =
		trainList.length != 0 ? trainList.map((train) => settings.trains[train]).reduce((train, sum) => sum + train) : 0
		...
		<div className="edit">
			...
			>
				...
				<div className="trains_buttons">
					{keys.map((key) => (
						<button className="trains_button" key={key} onClick={handleAddTrain}>
							{key}
						</button>
					))}
				</div>
...
const EditTickets = ({ tickets, setTickets, setTicketsScore, visibility, color }) => {
	...
	const ticketsScore = ticketList.length != 0 ? ticketList.reduce((ticket, sum = 0) => sum + ticket) : 0
			...
			>
				...
				<div className="tickets_group">
					...
					<div className="tickets_buttons">
						<button className="tickets_button tickets_button-done" onClick={() => handleAddTicket('+')}>
							Bilet ukończony
						</button>
						<button className="tickets_button tickets_button-failed" onClick={() => handleAddTicket('-')}>
							Bilet nieukończony
						</button>
					</div>
				</div>
...
const PlayerBox = ({ color, edit, name, remove, score, updateScore }) => {
	...
	return (
		<div className="player" style={{ boxShadow: `0px 0px 20px ${color}` }}>
			...
			<div className="player_remove" onClick={remove}>
				<i className="fa fa-trash" />
			</div>
			<div className="player_edit" onClick={edit}>
			...
			<div className="player_row">
				...
				<div className="player_row-edit" onClick={() => setEditTickets(true)}>
					<i className="fa fa-edit" />
				</div>
			</div>
			...
			<div className="player_row">
				...
				<div className="player_row-edit" onClick={() => setEditTrains(true)}>
					<i className="fa fa-edit" />
				</div>
			</div>
			...
			<div className="player_row">
				...
				<div className="player_row-edit" onClick={() => setEditBonus(true)}>
					<i className="fa fa-edit" />
				</div>
			</div>
...
// PlayerForm
const PlayerForm = ({ mode, player, setPlayer, onClick, players = [] }) => {
	React.useEffect(() => {
		ref.current.focus()
	}, [player])
	...
	const usedColors = players.filter((p) => p.id !== player.id).map((p) => p.color)
	...
	React.useEffect(() => {
		if (mode === 'add' && !isColorAvailable(player.color)) {
			const availableColor = Object.values(colors).find((color) => isColorAvailable(color))
			if (availableColor) {
				setPlayer({ ...player, color: availableColor })
			}
		}
	}, [players])
	...
	return (
		<>
			...
			>
				<option value={colors.black} disabled={!isColorAvailable(colors.black)}>
					Czarny {!isColorAvailable(colors.black) ? '(zajęty)' : ''}
				</option>
				<option value={colors.red} disabled={!isColorAvailable(colors.red)}>
					Czerwony {!isColorAvailable(colors.red) ? '(zajęty)' : ''}
				</option>
				<option value={colors.blue} disabled={!isColorAvailable(colors.blue)}>
					Niebieski {!isColorAvailable(colors.blue) ? '(zajęty)' : ''}
				</option>
				<option value={colors.green} disabled={!isColorAvailable(colors.green)}>
					Zielony {!isColorAvailable(colors.green) ? '(zajęty)' : ''}
				</option>
				<option value={colors.yellow} disabled={!isColorAvailable(colors.yellow)}>
					Żółty {!isColorAvailable(colors.yellow) ? '(zajęty)' : ''}
				</option>
			</select>
			<input
				ref={ref}
...
const App = () => {
	...
	const handleUpdatePlayer = () => {
		...
		setPlayers(
			players.map((player) => (player.id === editingPlayer.id ? editingPlayer : player)),
			// .sort((a, b) => a.name.localeCompare(b.name))
		)
		setEditingPlayer({})
	}
